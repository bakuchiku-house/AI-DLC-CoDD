---
codd:
  node_id: "skill:codd-scan"
  type: design
  depends_on: []
---

# CoDD Scan (Internal Document Index)

Rebuild the CoDD internal document index from frontmatter and source code so later commands (`codd validate`, `codd impact`, `codd generate`, `codd propagate`) operate on current project state.

> **AI-DLC × CoDD note**: In the 3-layer architecture, `codd scan` is CoDD's **internal plumbing** — it runs automatically via the PostToolUse hook after every file edit and does not need to be invoked manually in most cases. The user-visible knowledge graph is managed by **Graphify** (`graphify-out/graph.json`). Use `/graphify --update` for the user-facing graph. Use `/codd-scan` only when you need to force-refresh the CoDD index before running `codd impact` or `codd propagate`.

## When to Use

- Before `codd impact` or `codd propagate` when you need a guaranteed-fresh CoDD index
- When the PostToolUse hook was not triggered (e.g., files were copied externally)
- When scan counts look stale or unexpectedly low
- **Not needed for**: general codebase navigation (use Graphify instead)
