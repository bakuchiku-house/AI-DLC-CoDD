---
codd:
  node_id: "doc:code-generation-en"
  type: document
  depends_on: []
---
...
## [Three-Way Coherence Closure] Step 13.5: Completion Gate

**MANDATORY — Run after code generation, before presenting completion**:

```bash
# Step 1: Extract latest code state into CoDD (CoDD = primary design authority)
codd extract
# → Updates CoDD's knowledge of the generated codebase
# → Ensures design documents reflect actual generated code

# Step 2: CoDD internal coherence check (primary gate)
codd validate
# → Checks frontmatter, depends_on references, req↔design consistency
# → Exit 0: No issues — proceed
# → Exit 1: Issues found — include in warning block below (non-blocking for human)

# Step 3: Sync derived graph index
/graphify --update
# → Adds all generated source files and docs to graphify-out/graph.json
# → Re-runs Leiden community detection with updated node set
# → Generates GRAPH_REPORT.md for architectural context

# Step 4: CoDD/code↔Graphify coherence verification
/graphify query "Summarize coherence: coverage, missing links, key risks"
# → Verifies CoDD declared dependencies are traceable in the graph
```

**If coherence issues detected**, include the following warning block in the Step 14 completion message:
```
⚠️ Coherence Warning (Three-Way Coherence Closure)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
CoDD (primary): [codd validate issues, if any]
Graphify (derived): [query result summary]
Recommended action: [suggestion]
You may continue or resolve the issues first.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Optional — When design changes occurred during generation**:
```bash
# Auto-update affected design documents
codd propagate --update

# Show change impact bands (Green/Amber/Gray)
codd impact
```
