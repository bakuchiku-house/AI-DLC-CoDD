---
codd:
  node_id: "doc:requirements-analysis-en"
  type: document
  depends_on: []
---
...
### [CoDD] Step 7.5: Attach Frontmatter and Update Dependency Graph

**MANDATORY — Frontmatter on Requirements Document**:
Add CoDD frontmatter to `aidlc-docs/inception/requirements/requirements.md`:
```yaml
---
node_id: req-<project-slug>-v1
type: requirements
depends_on: []
...
**MANDATORY — Update Dependency Graph**:
```bash
codd scan --path .
# → Registers requirements.md in .codd/scan/nodes.jsonl and .codd/scan/edges.jsonl
```

**Optional — Generate Wave 1 Design Doc Template**:
```bash
codd generate --wave 1
# → Generates requirements-layer design document templates using the dependency graph
# → Useful as a starting context for subsequent phases (Greenfield projects)
```
