---
codd:
  node_id: "doc:units-generation-en"
  type: document
  depends_on: []
---
...
## [Graphify] Step 0.5: Community-Guided Decomposition (Brownfield)

**For Brownfield projects**: Before creating the unit plan, consult the knowledge graph communities as decomposition guidance.

```bash
# If graphify-out/graph.json exists from Workspace Detection / Reverse Engineering:
# Read graphify-out/GRAPH_REPORT.md for:
# → Community list (Leiden communities with member files — each is a candidate unit boundary)
# → God nodes (highest-connectivity nodes — span multiple communities = service boundaries)

# Use /graphify query for specific decomposition questions:
/graphify query "What are the natural module boundaries based on code coupling and design dependencies?"
```

**How to use community data for unit decomposition**:
- Each Leiden community = one candidate Unit of Work
- Communities with high internal cohesion but low inter-community edges = good unit boundaries
- God nodes that appear in multiple communities = integration/shared service candidates
- Communities with CoDD `depends_on` chains = likely ordered development sequence

**Record the community guidance in your decomposition rationale** in the unit-of-work plan.

> **Greenfield projects**: Skip this step — no existing code to analyze. Proceed to Step 1.
...
- **Reference Graphify communities** (if available) as the structural decomposition basis
